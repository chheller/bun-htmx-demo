...
export const createTodo = (db: Database, { todo }: Omit<Todo, "id">) =>
	db
		.prepare<TodoEntity, string>(
			"INSERT INTO todos (todo, complete) VALUES (?, false) RETURNING id, todo, complete",
		)
		.get(todo);
...
export const initializeTodoTable = (db: Database) =>
	db
		.query(
			"CREATE TABLE IF NOT EXISTS todos ( id INTEGER PRIMARY KEY AUTOINCREMENT, todo TEXT(128), complete INTEGER )",
		)
		.run();
